...
{
	//	已经到了最后一个边了，只需要检查他能不能原点即可 
	if (Depth == (G.e >> 1))
	{

		//if (G.Exist(Path.front(), Path.back()) && !visited[Path.front()][Path.back()])
	...
	*/
	if (1)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			Path.push_back(i);
			Euler(G, i, 0, visited, Path, Answer);
			Path.pop_back();


			for (size_t j = 0; j < Answer.size(); j++)
			{
				//assert(Answer[j].size() == m);
				Find = true;
				for (int k = 0; k < Answer[j].size() - 1; k++)
					std::cout << Answer[j][k] + 1 << "-->";
				std::cout << Answer[j][Answer[j].size() - 1] + 1 << std::endl;
			}
			Cnt += Answer.size();
		}
		if (Cnt)
			std::cout << "总共有" << Cnt << "种方式" << std::endl;
		if (!Find)
